...
void _makeNativeCrashReport(const char *reason, struct siginfo *siginfo, void *sigcontext) {
    ...
    if (result != JNI_OK)
        __android_log_print(ANDROID_LOG_ERROR, "NativeCrashHandler", "Could not attach thread to Java VM for crash reporting.\nCrash was: %s", reason);
...
JNIEXPORT jboolean
JNICALL Java_com_shellware_nativecrashhandler_NativeCrashHandler_nRegisterForNativeCrash(JNIEnv *env,
                                                                                 jobject obj) {
...
Java_com_shellware_nativecrashhandler_NativeCrashHandler_nUnregisterForNativeCrash(JNIEnv *env, jobject) {
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    applicationClass = env->FindClass("com/shellware/nativecrashhandler/NativeCrashHandler");
